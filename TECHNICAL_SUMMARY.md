...
**规模**：新增 **2 个 Java 类**（`.java` **55 → 57**）+ **1 个仪式配方 JSON**；
...
**配置 71 → 72 项**（`[servant]` 段 **3 → 4**，新增 `wandBlessingEnabled`；段数仍 **11**）；
jar 条目 **117 → 120**（新增 3 = 2 个新 class + `tuner_servant_ritual.json`，**无删除**）；
网络协议仍 **2.1**（未动）。
...
- **部署核对**：构建产物与部署 jar **逐字节相同**
  （1,793,394 B / md5 `81B466A86D59497793D84A8A138246F0` / 120 条目）。
...
| 0.0.20 | `goetytuner-0.0.20.jar` | 1,793,394 B | `81B466A86D59497793D84A8A138246F0` | `versions\测试\mods\`（该目录只保留这一个 goetytuner jar） |
